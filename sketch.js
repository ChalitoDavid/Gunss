...
function preload(){
  ...
  gunAnimation = loadAnimation("Gun(1).png",
  "Gun(3).png","Gun(6).png","Gun(2).png")
...
function setup() {
  ...
  gun.scale=0.5
  gun.addAnimation("gun1", gunImg)
  
  gun.addAnimation("gun2", gunAnimation);
...
function draw() {
  ...
  if(gameState===1){
    ...
    if(keyDown("space") && count === 0){
      gunSound.play();
      gun.changeAnimation("gun2", gunAnimation);
      fix= true;
      shootBullet(); 
    }
    if(fix===true){
      count +=1;
      if(count >= 10){
        gun.changeAnimation("gun1", gunImg);
        count=0;
        fix = false; 
      }
    }
